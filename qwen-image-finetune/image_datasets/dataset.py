...
class CustomImageDataset(Dataset):
    def __init__(self, img_dir, json_path, img_size=512, caption_type='txt',
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.cached_image_embeddings is None and self.img_cache_dir is None:
                img = Image.open(self.img_dir+self.images[idx]['image']).convert('RGB')
                ...
                img = torch.load(os.path.join(self.img_cache_dir, self.images[idx]['image'].split('/')[-1] + '.pt'))
            else:
                img = self.cached_image_embeddings[self.images[idx]['image'].split('/')[-1]]
